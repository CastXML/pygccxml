...
class scanner_t(xml.sax.handler.ContentHandler):
    ...
    def __read_location_bootstrap(self, inst, decl, attrs, _):
        ...
        if self.xml_generator.is_castxml:
    ...
    def __read_offset_type(self, attrs):
        ...
        type_ = attrs[XML_AN_TYPE]
        if self.xml_generator.is_castxml or \
            self.xml_generator.is_gccxml_09 or \
                self.xml_generator.is_gccxml_09_buggy:
    ...
    def __read_argument(self, attrs):
        if isinstance(self.__inst, declarations.calldef_type_t):
            ...
            if self.xml_generator.is_gccxml:
