...
def test_on_windows_dot_h():
    ...
    reader = parser.source_reader_t( parser.gccxml_configuration_t(gccxml_path=autoconfig.gccxml_path), dcache )
...
def test_project_on_include_std_dot_hpp():
    ...
    clock_prev = time.clock()
    dcache = parser.file_cache_t(dcache_file_name)
    reader = parser.project_reader_t( parser.gccxml_configuration_t(gccxml_path=autoconfig.gccxml_path), dcache )
...
def profile_project():
    include_std_header = os.path.join( autoconfig.data_directory, 'include_std.hpp' )
    reader = parser.project_reader_t( parser.gccxml_configuration_t(gccxml_path=autoconfig.gccxml_path) )
    reader.read_files([include_std_header])
