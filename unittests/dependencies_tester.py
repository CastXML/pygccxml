...
class tester_t( parser_test_case.parser_test_case_t ):
    ...
    def test_class( self ):
        ...
        if '0.9' in cls.compiler:
            ...
            dependencies = [d for d in dependencies if not d.declaration.is_artificial]
            self.failUnless( len(dependencies) == 1 )
        else:
            self.failUnless( len(dependencies) == 2 ) #compiler generated copy constructor

        m_mutable = ns_vars.variable( 'm_mutable' )
        dependencies = [dependency for dependency in dependencies if dependency.declaration is m_mutable]
        self.failUnless( len(dependencies) == 1 )
        self.failUnless( dependencies[0].depend_on_it.decl_string == 'int' )
        ...
        dependencies = [dependency for dependency in dependencies if dependency.depend_on_it is derived1_cls]
        self.failUnless( len(dependencies) == 1 )
    ...
    def test_calldefs( self ):
        ...
        used_types = [dependency.depend_on_it.decl_string for dependency in dependencies]
        ...
        self.failUnless( len(dependencies) == 3 )
        dependencies = [dependency for dependency in dependencies if dependency.depend_on_it in ( some_exception, other_exception )]
